...
import os
import sys
from Unsupervised import summarize
import matplotlib.pyplot as plt
from moviepy.editor import VideoFileClip, concatenate_videoclips

def demo(video_name='static\\input.mp4', summ_ratio=0.2):
    SUMM_RATIO = 0.1  # The minimum allowed ratio between the summary video and the full video.
    ...
    # Calculating allowed budget
    budget = float(summ_ratio) * np.sum(C)

    # Use ILS_SUMM to obtain a representative subset tht satisfies the knapsack constraint.
    representative_points, total_distance = summarize(X, C, budget)
    ...
    # Generating the video summary file
    ...
    if chosen_shots_clips == []:
        ...
        summ_clip.write_videofile(os.path.join('static', "output.mp4"))
